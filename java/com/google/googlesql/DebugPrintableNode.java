    ...
    if (fields.isEmpty()) {
      ...
      for (DebugStringField field : fields) {
        ...
        if (!printOneLine) {
          ...
          boolean isLastField = field == Iterables.getLast(fields);
          for (DebugPrintableNode node : field.nodes) {
            Preconditions.checkState(node != null);
            String fieldNameIndent = printFieldName ? (isLastField ? "  " : "| ") : "";
            boolean isLastNode = node == field.nodes.get(field.nodes.size() - 1);
            String fieldValueIndent =
                (!isLastNode || (!printFieldName && !isLastField) ? "| " : "  ");
