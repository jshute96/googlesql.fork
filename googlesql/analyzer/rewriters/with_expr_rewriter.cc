...
WithExprRewriterVisitor::MakeProjectionForComputedColumn(
    const ResolvedComputedColumn& column,
    std::unique_ptr<ResolvedScan> input_scan) {
  GOOGLESQL_ASSIGN_OR_RETURN(auto processed, ProcessNode(&column));
