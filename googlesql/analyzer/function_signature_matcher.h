...
#ifndef GOOGLESQL_ANALYZER_FUNCTION_SIGNATURE_MATCHER_H_
#define GOOGLESQL_ANALYZER_FUNCTION_SIGNATURE_MATCHER_H_
...
// Callback used to look up a generated proxy function by its name.
// This is used during signature matching when a function reference is passed
// where a lambda is expected. Looking up the generated function allows us to
// retrieve its signature, enabling type inference for template arguments of the
// outer function (e.g., inferring the return type of ARRAY_TRANSFORM when
// passed a function reference).
using LookupGeneratedFunctionCallback =
    absl::AnyInvocable<const Function*(IdString) const>;
...
// Wrapper for FunctionSignatureMatchesWithStatus for backward compatibility.
// New callers should use the version with lookup_generated_function_callback.
absl::StatusOr<bool> FunctionSignatureMatchesWithStatus(
    const LanguageOptions& language_options, const Coercer& coercer,
    const std::vector<const ASTNode*>& arg_ast_nodes,
    absl::Span<const InputArgumentType> input_arguments,
...
#endif  // GOOGLESQL_ANALYZER_FUNCTION_SIGNATURE_MATCHER_H_
