...
const SourceLocation kLoc = GOOGLESQL_LOC;
...
TEST(StatusBuilderTest, Ctors) {
  EXPECT_EQ(ToStatus(StatusBuilder(kZomg, GOOGLESQL_LOC) << "zomg"),
            absl::Status(kZomg, "zomg"));
}

TEST(StatusBuilderTest, Identity) {
  SourceLocation loc(GOOGLESQL_LOC);
...
TEST(CanonicalErrorsTest, CreateAndClassify) {
  for (const auto& test : kCanonicalErrorTests) {
    ...
    absl::Status status = test.creator(GOOGLESQL_LOC) << message;
